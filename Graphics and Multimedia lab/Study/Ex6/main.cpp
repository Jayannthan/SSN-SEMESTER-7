...
void plotPoints(float arr[][3], int n)
{
	for (int i = 0; i < n; i++)
	{
		stringstream s, s2;
		s << arr[i][0];
		s2 << arr[i][1];
		drawString(arr[i][0], arr[i][1], ("(" + s.str() + "," + s2.str() + ")").c_str());
	}
}
...
void matCopy(float a[][3], float mult[][3], int n)
{
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < 3; j++)
...
void shearing(float arr[][3], int n, float s, float mul[][3], int choice)
{
	if (choice == 1)
	{
		float b[3][3] = {{1, 0, 0}, {s, 1, 0}, {0, 0, 1}};
		matMul(arr, b, mul, n);
	}
	else
	{
		float b[3][3] = {{1, s, 0}, {0, 1, 0}, {0, 0, 1}};
		matMul(arr, b, mul, n);
	}
}
...
void myDisplay()
{
	...
	do
	{
		...
		cout << "Apply more transformation? (y/n) : ";
		cin >> choice;
	} while (choice == 'y');
	pltFig(arr, n);
	plotPoints(arr, n);
